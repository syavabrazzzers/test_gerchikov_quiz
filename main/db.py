...
class Database:
    ...
    def post_result(self, *args):
        ...
        try:
            ...
            with self.connection:
                self.cursor.execute(
                    f"UPDATE users SET {string[:-2]} WHERE chat_id={l[0]}", new_l
                )
    ...
    def isMessageExists(self, key):
        with self.connection:
            res = self.cursor.execute(
                "SELECT COUNT(*) as cnt FROM bookmarks WHERE `key` = ?", (key,)
            ).fetchone()[0]
        return res
    ...
    def get_key(self, from_id, key):
        res = self.cursor.execute(
            "SELECT message_id FROM bookmarks WHERE `key` = ? AND from_id = ?",
            (key, from_id),
        ).fetchone()[0]
        return res
    ...
    def list_key(self, from_id):
        res = self.cursor.execute(
            "SELECT `key` FROM bookmarks WHERE `from_id` = ?", (from_id,)
        ).fetchall()
        return res
    ...
    def remove_key(self, from_id, key):
        ...
        if not res:
            return
        res = self.cursor.execute(
            "DELETE FROM bookmarks WHERE `key` = ? AND from_id = ?", (key, from_id)
        )
        return res
