...
from gerchikov.handlers import get_result, add_points
...
async def set_position(message: Message, state: FSMContext):
    ...
    await state.update_data(max_choices=first_question_key.choices_count, question=0, answers=[])
    await message.answer(first_question_key.text, reply_markup=get_quiz_markup(first_question_key.questions))


@router.callback_query(StateFilter(GerchikovState.user_naber), CallbackTextFilter('next_question'))
...
async def question_answer(callback: CallbackQuery, state: FSMContext):
    ...
    if int(callback.data) in state_answers:
        ...
        await bot.send_message(
            ...
            reply_markup=get_quiz_markup(questions[current_question].questions, state_answers)
        )
        return
    ...
    await bot.send_message(
        callback.message.chat.id,
        questions[current_question].text,
        reply_markup=get_quiz_markup(questions[current_question].questions, state_data['answers'])
    )
